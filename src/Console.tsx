...
export default function Console() {
  ...
  const exportToGitHub = async () => {
    ...
    if (!token) { setError("GitHub authorization failed or was cancelled."); return; }
    ...
    try {
      const createRes = await fetch("https://api.github.com/user/repos", {
        ...
        body: JSON.stringify({ name: repoName, description: `SDK generated by SDKCraft for ${result.title}`, private: false, auto_init: true }),
      });
...
>SDKCraft</span>
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: "12px", flexWrap: "wrap" }}>
          ...
          {user && (
            <button onClick={handleLogout} style={{ background: "none", color: "#aaa", border: "1px solid #333", padding: "6px 14px", borderRadius: "8px", fontSize: "13px", cursor: "pointer" }}>
              Logout
            </button>
          )}
        </div>
